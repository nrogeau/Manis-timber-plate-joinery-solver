...
__funding__ = "NCCR Digital Fabrication"
__version__ = "2024.08"
__license__ = "MIT License"
__repository__ = "https://github.com/nrogeau/Manis-timber-plate-joinery-solver"
__reference__ = "https://doi.org/10.1016/j.autcon.2021.103875"
...
class PlateModel:
    ...
    def __set_sequence(self, sequence):
        ...
        if sequence == 0 or sequence == [] or sequence == None:
            ...
            if type(sequence) is str:
                ...
                if(len(Toolbox.Data.flatten_integer_list(test)))>(self.count):
                    raise Exception('The provided sequence is longer than the list of breps.')
                return sequence
    ...
    def __reorder_breps(self, breps):
        ...
        flat_seq = Toolbox.Data.flatten_integer_list(seq)
        for i in flat_seq:
            ordered_breps.append(breps[i])
        return ordered_breps
        #Toolbox.Data.sort_list_sync(breps, Toolbox.Data.flatten_integer_list(seq))
        ...
        for i in range(self.count):
            ...
            for j in range(len(self.contact_types[i])):
                ...
                if constraints[2] == [] and (self.contact_types[i][j] == 'ES' or self.contact_types[i][j]  == 'SE'):
                    ...
                    disk = rs.AddPlanarSrf(rs.AddCircle(trim_plane,2))
                    insertion_space = rg.Brep.Trim(rs.coercebrep(insertion_space),rs.coercebrep(disk),1)[0]
                    insertion_space = scriptcontext.doc.Objects.Add(insertion_space)
                    self.temp.append(insertion_space)
    ...
    def __get_assembly_vectors(self):
        ...
        sub_seq = []
        for i in range(len(steps)):
            if steps[i] not in Toolbox.Data.deepest_steps(seq):
                sub_seq.append(Toolbox.Data.get_item_from_path(seq, steps[i]))
        sub_seq.append(seq)
                    ...
                    if search is True:
                        ...
                        if (i in plates_in_sequence) and (adj[i][j] in plates_in_sequence):
                            for l in  range(len(mod_seq)):
                                ...
                                if corresponding_vector != "gravity":
    ...
    def intersect_insertion_spaces(self, insertion_spaces):
        ...
        if len(pts) != 0:
            dso = 0
            candidates.append(rg.Point3d(rs.PointCoordinates(pts[0])))
            #check points
    ...
    def add_dowels(self, 
        ...
        if dowel_number == 0: dowel_number = 1
        if dowel_radius == 0: dowel_radius = 1
    ...
    def add_tenons(self, 
        ...
        if tenon_number == 0: tenon_number = 1
        if tenon_width == 0: tenon_width = 1
    ...
    def add_fingers(self, 
        ...
        if finger_number_1 == 0: finger_number_1 = 1
        if finger_width_1 == 0: finger_width_1 = 1
        if finger_number_2 == 0: finger_number_2 = 1
        if finger_width_2 == 0: finger_width_2 = 1
    ...
    def add_halflap(self,
        ...
        if proportion == 0 or proportion == 1: proportion =0.5
        ...
        for i in range(self.count):
            ...
            for j in range(len(types)):
                ...
                if (types[j] == 'IN') and (nb > i) and (i_want_a_halflap is True):
                    ...
                    self.plates[i].joints_negatives.append(rs.coercebrep(pieces[0]))
                    self.plates[nb].joints_negatives.append(rs.coercebrep(pieces[1]))
...
class Plate:
    ...
    def __get_top_contour(self):
        ...
        if type(largest_contour) != rg.PolylineCurve:
            largest_contour=largest_contour.ToPolyline(0.01,0.01,0.01,10000)
...
class Toolbox:
    ...
    class Breps:
        ...
        @staticmethod
        def discretizeCurvedBreps(breps, length=1):
            def FaceToPolyline(face):
                points = []
                contour = Toolbox.Surfaces.get_face_largest_contour(face)
                if type(contour) is rg.PolyCurve: segments = contour.Explode()
                else: segments = [contour]
                for segment in segments:
                    if type(segment) is rg.PolylineCurve:
                        points += segment.ToPolyline().ToArray()[0:-1]
                    elif rg.Curve.IsLinear(segment) is True:
                        points += [segment.PointAtStart]
                    else:
                        points += [segment.PointAt(param) for param in segment.DivideByLength(length, True)]
                return points
            ...
            for j in range(len(breps)):
                ...
                sortedfaces = Toolbox.Surfaces.sort_surfaces_by_area(faces)
                sortedfaces.reverse()
                top, bottom = sortedfaces[0][0], sortedfaces[1][0]
                bottom.Flip()
                top_points = FaceToPolyline(top)
                bottom_points = FaceToPolyline(bottom)
                
                for i in range(len(bottom_points)):
                    dist = top_points[0].DistanceTo(bottom_points[i])
                    if i == 0:
                        best_dist = dist
                        best_i = 0
                    elif best_dist > dist:
                        best_dist = dist
                        best_i = i
                bottom_points = bottom_points[best_i:] + bottom_points[:best_i]
                if len(bottom_points) != len(top_points): raise Exception('Could not discretize brep '+str(j)+' as top and bottom contours do not match.') 
                sides=[]
                for i in range(len(top_points)):
                    sides.append(rg.NurbsSurface.CreateFromCorners(top_points[i-1], bottom_points[i-1], bottom_points[i],top_points[i]))
                sides = [rg.Brep.CreateFromSurface(side) for side in sides]
                top = rg.Brep.CreatePlanarBreps(rg.Polyline(top_points+[top_points[0]]).ToNurbsCurve())[0]
                bottom = rg.Brep.CreatePlanarBreps(rg.Polyline(bottom_points+[bottom_points[0]]).ToNurbsCurve())[0]
                new_breps.append(rg.Brep.JoinBreps(sides+[top, bottom], 0.001)[0])
            return new_breps
    ...
    class Curves:
        ...
        def resimplify_Curve(curve):
            ...
            try:
                ...
                raise Exception('Plate contour is not a polyline, curves need to be discretized.')
